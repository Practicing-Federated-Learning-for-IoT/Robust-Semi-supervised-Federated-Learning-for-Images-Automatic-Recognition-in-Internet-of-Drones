...
class TrainManager:
    ...
    def train_one_round(self, curr_round, client_id ,round_cnt, curr_task):#lss->client_id
        ...
        self.opt.lambda_1 =(2/(math.pi)) * math.atan((self.opt.frac_clients * 100 * curr_round)/(2*bsize_u * 1+1))
        self.opt.lambda_2 = 1 - self.opt.lambda_1
        #print("lambda_1={},lambda_2={}".format(self.opt.lambda_1,self.opt.lambda_2))
        ...
        for epoch in range(self.params['num_epochs']):
            ...
            for i in range(num_steps):
                if 'fedmatch' in self.opt.model:
                    ...
                    y_unlabeled = self.task['y_unlabeled'][i*bsize_u:(i+1)*bsize_u] 
                    x_unlabeled = self.task['x_unlabeled'][i*bsize_u:(i+1)*bsize_u] 
                    with tf.GradientTape() as tape:
                        ######################################
                        #       unsupervised learning    
                        ######################################
                        _, loss_u, num_conf = self.params['loss_u'](x_unlabeled,y_unlabeled)#here altered in 2021-8-16:u->s
