...
class Client:
    ...
    def loss_u(self, x, y):
        ...
        for i in range(len(y_pred)):
            tmp = -tf.reduce_sum(y_pred[i] * tf.math.log(y_pred[i]))
            loss_e.append(tmp)
        rank = [index for index, value in sorted(list(enumerate(loss_e)), key=lambda x:x[1])]
        rank = np.array(rank)
        if len(rank)>20:
            ...
            nrank = rank
        if len(rank):
            x_conf = self.data_manager.rescale(x[nrank])
            y_pred_e = K.gather(y_pred, nrank)
        #conf = np.where(np.max(y_pred.numpy(), axis=1)>=self.opt.confidence)[0]
        #if len(conf)>0:
            #x_conf = self.data_manager.rescale(x[conf])
            y_pseu = np.zeros([len(y_pred_e),len(y_pred_e[0])])
            for i in range(3):
                y_pseu += self.local_model(self.data_manager.rescale(self.data_manager.augment(x[nrank], soft=True)))
            ...
            #self.check_entropy(y_pseu,y[nrank])
            #self.check_entropy(y_pseu,y[conf])
            loss_u += self.cross_entropy(y_pseu, y_pred_e) * self.opt.lambda_1
        # additional regularization
        for lid, psi in enumerate(self.psi):
            #l2 regularization
            loss_u += tf.math.reduce_sum(tf.math.square(self.sigma[lid]-psi)) * self.opt.lambda_l2
        return y_pred, loss_u, len(nrank)

    def check_entropy(self, pse, label):
        count = 0
        for i in range(len(pse)):
            if(np.argmax(pse[i])==np.argmax(label[i])):
                count = count + 1
        tmp = count/len(pse)
        acc.append(tmp)
